...
установщика не доказано, продолжение запрещено: оболочка печатает
`SOURCE_UPDATE_CLEANUP_REQUIRED`, возвращает код 69 и не открывает постоянный
шлюз на снимке. Это отдельный строгий отказ, а не временный `RETRY_AFTER`.
...
| `SOURCE_UPDATE_CLEANUP_REQUIRED` | Исчезновение группы процессов установщика не доказано | Запуск уже остановлен кодом 69; не открывать параллельный управляемый сеанс, сохранить диагностику и повторить `--doctor --json` после завершения оставшегося процесса |
...
сведений нет или нет обеих пар, обязательный управляемый запуск завершается
кодом 69 и предлагает `codex-native`; скрытого перехода на обычный путь нет.
