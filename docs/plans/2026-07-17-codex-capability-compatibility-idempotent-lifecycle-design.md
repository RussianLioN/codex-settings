# Совместимость Codex по возможностям и идемпотентный жизненный цикл
...
- договоры варианта 2 и планируемого выпуска `0.2.0` в этой ветке образуют
  проверяемый проект реализации, но ещё не работающий маршрут;
- исполняемая реализация `0.2.0`, чистая установка, испытания всех аварийных
  окон и пробное включение маршрута остаются последующими этапами.

Документ не утверждает текущую версию локального Codex. Приведённые ниже
версии и состояние исполняемого файла относятся только к снимку наблюдений от
17 июля 2026 года и перед любой установкой должны проверяться заново.

## Результат решения

После реализации варианта 2 адаптивные субагенты не должны считать точный
номер Codex единственным доказательством совместимости. Установщик должен
создавать частный неизменяемый снимок выбранного исполняемого файла, проверять
подпись, интерфейс и поведение, а контроллер — перед каждой границей запуска
заново получать фактически доступные модели и уровни рассуждения.
...
Исторический снимок наблюдений на 17 июля 2026 года:

- наблюдавшаяся стабильная версия — Codex `0.144.5`;
- наблюдавшийся локальный `/opt/homebrew/bin/codex` сообщал
  `codex-cli 0.144.5`;
- для того исторического снимка обновлять сам Codex не требовалось;
- изменение `0.144.4 → 0.144.5` ограничено исправлением распознавания опасных
  команд удаления и повышением номера выпуска;
...
- [выпуск Codex 0.144.5](https://github.com/openai/codex/releases/tag/rust-v0.144.5);
- [сравнение 0.144.4 и 0.144.5](https://github.com/openai/codex/compare/rust-v0.144.4...rust-v0.144.5);
...
flowchart TD
    ...
    F --> S[Рубеж 1: smart_start создаёт admissionId]
    S --> E[Свежий AccountEvidence без кэша]
    E --> P[Рубеж 2: reserve_launch_permit сверяет шлюз и эпоху]
...
На всех трёх рубежах проверяется один и тот же канонический объект
...
```mermaid
flowchart TD
    I[Проверка перед smart_start] --> J{Журнал существует или отсутствие не синхронизировано?}
...
закрывают умный путь: обычный Codex остаётся доступным, а `smart_start` не
создаёт `admissionId`. Вычислять шлюз и начинать тройной допуск можно только
при совпавшей неизменяемой квитанции и полном свежем доказательстве
синхронизированного отсутствия основного журнала.
...
Помета «договор готов» означает только наличие проверяемых документов, схем и
векторов в этой ветке. Она не означает наличие исполняемого маршрута
`0.2.0`. Все пункты с пометой «предстоящая реализация» требуют кода и живой
проверки.
...
### Этап 2. Убрать жёсткую версию Codex — предстоящая реализация
...
### Этап 3. Ввести загрузочный шлюз и активации — предстоящая реализация
...
### Этап 4. Обновить контроллер и SQLite — предстоящая реализация
...
- реализовать `smart_start`, свежий `AccountEvidence`,
...
### Этап 5. Реализовать переход и восстановление — предстоящая реализация
...
Статус этапа: предстоящая реализация.
...
- закреплённый исторический снимок Codex `0.144.5` проходит эталонные
...
- `smart_plan`, `smart_start`, допуск узла и каждый `Popen` используют свежую
  полную проверку требований и моделей;
...
- `smart_start` не создаёт `admissionId`, пока журнал существует или его
  отсутствие не доказано синхронизированным;
- `smart_start`, `reserve_launch_permit` и `commit_launch_permit` сверяют
  канонически одинаковый `activationGate`; `ACCEPTING` без полного шлюза
  недостаточно;
...
Текущая ветка доказывает проект договора, но не готовность выпуска. До
объявления `0.2.0` ещё нужны исполняемый код, независимая проверка исходников и
тестов, чистая установка, испытания аварийных окон, `doctor`, проверка,
пробное включение и откат.
