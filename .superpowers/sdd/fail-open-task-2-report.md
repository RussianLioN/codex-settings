...
После коммита реализации `bfcee3f` выполнены:
...
- Сам отчёт фиксируется отдельным атомарным коммитом после повторной проверки.
...
- Набор проверок выводит неблокирующее `ResourceWarning` о соединении SQLite
  из существующего `schema_projection.py`; все 109 проверок завершаются
