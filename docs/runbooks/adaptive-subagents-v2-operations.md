...
Если проверка до создания контекста не сошлась, это не авария запуска: текущий
запрос обрабатывается как обычный и `TurnContext` не создаётся. Если уже после
этого не запустился ленивый MCP, `Stop` не образует бесконечный цикл: после
двух напоминаний завершение разрешается. При неожиданном обычном ходе сначала
