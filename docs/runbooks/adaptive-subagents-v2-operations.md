...
Команда `codex` не заменяется. Установщик регистрирует локальный рынок и
расширение штатными командами Codex и записывает квитанцию, связанную с
абсолютными путями, активацией и отпечатком входов установки. В отпечаток
входят исходники, лексический путь и содержимое выбранного Codex, а также
...
## 5. Обычный и адаптивный запуск

Обычный Codex без маршрутизации:
...
```bash
~/.local/bin/codex-smart -C /путь/к/репозиторию
```

Если модель основного диалога не задана, оболочка добавляет
...
узлов всё равно выбирает контроллер. Подкоманды, справочные вызовы и
неподдержанные параметры выполняются как обычный Codex.

Даже для поддержанного интерактивного вызова адаптивный режим включается
только после трёх проверок: неизменяемый `.mcp.json` объявляет ровно четыре
обязательных инструмента с режимом `approve`; пользовательская политика не
...
Перед созданием контекста хода дополнительно проверяется живой контроллер.

Если любая проверка не сошлась, это не авария запуска: текущий запрос
обрабатывается как обычный, `TurnContext` не создаётся, а `Stop` не требует
вызвать недоступный `smart_plan`. При неожиданном обычном ходе сначала
