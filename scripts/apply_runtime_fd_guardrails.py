...
def desired_config(text: str) -> str:
    tomllib.loads(text)
    text = upsert_table_integer(text, "agents", "max_threads", SAFE_THREAD_CAP)
    text = upsert_table_integer(
        text,
    ...
    if parsed["agents"]["max_threads"] != SAFE_THREAD_CAP:
        raise ValueError("agents.max_threads repair failed")
    if (
        parsed["features"]["multi_agent_v2"]["max_concurrent_threads_per_session"]
        != SAFE_THREAD_CAP
    ):
        raise ValueError("native session thread cap repair failed")
    return text
...
def state_issues(
    *,
    ...
    if parsed.get("agents", {}).get("max_threads") != SAFE_THREAD_CAP:
        issues.append("agents_max_threads_not_20")
    if (
        parsed.get("features", {})
        .get("multi_agent_v2", {})
        ...
        issues.append("native_session_thread_cap_not_20")
...
def main() -> int:
    ...
    for path in (config_path, agents_path, args.source_doctor):
        if not path.is_file():
