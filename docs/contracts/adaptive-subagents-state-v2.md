...
Актуальная граница на 14 августа 2026 года: договор описывает целевое
поведение кандидата, но не подтверждает действующую рабочую среду.
`HOOK-INCIDENT-001` остаётся блокирующим P0: разбор первопричин частичный,
этапы 1–4 и живая приёмка не закрыты. В основном профиле действует
`[features] hooks = false`; хуки и смарт-запуск выключены. До закрытия
инцидента и отдельного решения владельца этот договор не разрешает запуск
установки, доверие хукам или согласование точки входа с `--apply`.
