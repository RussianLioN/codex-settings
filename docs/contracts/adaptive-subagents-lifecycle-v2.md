...
Установщик хранит монотонную `sourceLineage` реализации. Закрытый объект
содержит `schemaVersion=1`, положительное целое `generation` и полный
`implementationDigest`. Для обновления и отката он входит в
`manifestDocument`, commit-квитанцию и производную квитанцию установщика. При
первой установке исходный материализатор фиксирует initial-манифест до
появления квитанции установщика, поэтому его `extensions` может оставаться
пустым: тогда линия закрепляется в квитанции и независимо сверяется с её
копией в неизменяемой активной активации. Первый последующий переход уже
переносит её в манифест и commit-квитанцию.

При аварии между commit манифеста обновления или отката и заменой производной
квитанции восстановитель обязан взять `sourceLineage` только из принятого
`manifestDocument`. Живой исходный каталог не является источником истины для
этого доведения.

Меньшее поколение не может начать обновление, а один номер поколения не может
обозначать два разных отпечатка реализации. Историческое отсутствие поля
допускается только как одноразовая граница перехода; после публикации
непустого `extensions.sourceLineage` установщик старого договора не имеет
права изменять глобальную установку.
