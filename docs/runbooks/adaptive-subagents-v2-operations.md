...
Статус на 14 августа 2026 года: эта инструкция описывает будущую
изолированную приёмку кандидата после снятия блокировки, а не текущий рабочий
профиль. `HOOK-INCIDENT-001` остаётся блокирующим P0; разбор первопричин
частичный, этапы 1–4 и живая приёмка не закрыты. В основном профиле действует
`[features] hooks = false`; хуки и смарт-запуск выключены. До отдельного
решения владельца запрещено выполнять установку с `--apply`, принимать хуки,
запускать согласование точки входа с `--apply` или считать команды руководства
разрешением на включение.
...
2. Канонический стабильный номер Codex не ниже `0.144.4` только допускает
   выпуск к проверке возможностей. Он не является безусловной гарантией
   совместимости: до принятия сверяются интерфейс, каталог моделей, политика
   и жизненный цикл. Неканонический, предварительный или слишком старый
   выпуск закрывает адаптивный путь.
...
6. Для дочернего запуска использовать канонический корень Git с зафиксированным
   ...
   подмодули. Соседние зарегистрированные рабочие копии Git допустимы, если
   текущий канонический путь встречается в `git worktree list` ровно один раз.
   Контроллер отдельно проверяет исходное дерево, индекс, ссылки и ресурсные
   пределы перед каждым запуском.
...
Каждое намеренное изменение устанавливаемой реализации требует увеличить
`generation` в
`plugins/codex-smart-subagents/config/source-lineage-v2.json` и записать
пересчитанный `implementationDigest`. Обновление только двоичного файла Codex
поколение не меняет. Запуск из старой рабочей копии после защищённой установки
заканчивается закрытым отказом и не заменяет активную активацию.
...
- `READY` — активация, выбранная пара координатора и живой командный сокет
  подтверждены, а последняя проверка каталога успешна;
- `AWAITING_HOOK_TRUST` — установка готова, но Codex ожидает явного решения
  пользователя о доверии хукам;
- `DEGRADED` — основной Codex доступен, но полная готовность умной части не
  доказана. При начальном `UNAVAILABLE` умной пары ещё нет; после прежнего
  `SELECTED` доказанная пара сохраняется, но свежесть каталога временно не
  подтверждена;
...
`--version`, вызывает `hooks/list`, проверяет фактический `sourcePath` кеша и
не отправляет пользовательскую задачу модели. Успешный структурированный
ответ содержит `hookSourcePath`.

В `extensions.diagnosis` команды `--doctor --json` доступны безопасные поля
`coordinatorReasonCode`, `lastSuccessfulCheckAt` и `nextAttemptAt`.
Канонические метки времени имеют часовой пояс UTC. Пути, тексты исключений и
содержимое каталога моделей в эту проекцию не входят. После неудач один
фоновый цикл повторяет проверку через 5, 30, 120 и 300 секунд; после успеха —
раз в 300 секунд. Повреждённое либо отсутствующее расширение не даёт ложный
`READY`, а дымовая проверка в `DEGRADED` завершается отказом полной готовности.
...
Если системный Codex был заменён, первый новый управляемый интерактивный
запуск `codex` проверяет целостность принятой активации и снимка. При
доказанной активации он остаётся в `READY` на последнем проверенном
неизменяемом снимке и фиксирует `source_drift`; новый номер сам по себе не
делает системный файл совместимым.

Закрытая квитанция и отдельная блокировка разрешают не более одного процесса
согласования одновременно. Согласователь запускается только из неизменяемой
капсулы принятой активации, без зависимости от исходной рабочей копии
репозитория. После временного отказа та же личность источника не допускается к
повтору раньше чем через 300 секунд. При `ACCEPTED` оболочка ровно один раз
перезапускает стабильную управляемую команду. При `INCOMPATIBLE` либо
`RETRY_AFTER` текущий управляемый запуск продолжается на снимке. Если
перезапущенный процесс видит защитный маркер, `RESTART_GUARD` запрещает вторую
попытку в этом логическом запуске и также продолжает снимок.

Если после превышения времени или объёма вывода исчезновение группы процессов
установщика не доказано, умное продолжение запрещено: оболочка печатает
`SOURCE_UPDATE_CLEANUP_REQUIRED` и передаёт обычный запуск проверенному
основному Codex. Это отдельное ограничение умной части, а не временный
`RETRY_AFTER`.

Сообщения оболочки и действие оператора:

| Сообщение | Значение | Действие |
|---|---|---|
| `SOURCE_UPDATE_INCOMPATIBLE` | Новый системный Codex не прошёл обязательную проверку возможностей | Продолжить текущую сессию на снимке; сохранить диагностику и не объявлять выпуск совместимым |
| `SOURCE_UPDATE_RETRY_AFTER` | Попытка временно не может быть доказана; квитанция запрещает следующий процесс минимум на 300 секунд | Продолжить на снимке; не запускать согласователь вручную, следующий обычный запуск можно проверить не раньше чем через пять минут |
| `SOURCE_UPDATE_RESTART_GUARD` | Один перезапуск уже был выполнен в этом запуске | Продолжить на снимке; не создавать вторую попытку или второй перезапуск |
| `SOURCE_UPDATE_CLEANUP_REQUIRED` | Исчезновение группы процессов установщика не доказано | Умная активация не используется; основной Codex остаётся доступен. Не открывать параллельный управляемый сеанс, сохранить диагностику и повторить `--doctor --json` после завершения оставшегося процесса |

Проверить наблюдаемое состояние без ручного запуска согласователя:
...
Первая команда должна показать текущий системный выпуск, потому что
`codex-native` всегда обходит контроллер. Вторая проверяет квитанцию,
привязку активации и снимок; третья показывает состояние действующего
контроллера. Эти команды сами по себе не доказывают живую проверку
самовосстановления. Не заменять ими обычный новый интерактивный запуск
`codex`, когда требуется наблюдать эту ветвь.
...
Псевдоним `codex` выставляет `CODEX_SMART_ENABLED=1`, затем передаёт запуск в `codex-highfd`.
...
Если корневая модель и уровень рассуждений не заданы явно, управляемая
оболочка один раз читает свежие сведения учётной записи и предпочитает
`gpt-5.6-sol` с `medium`. Если Sol недоступна, она ровно один раз выводит:
`codex-smart: COORDINATOR_PAIR_FALLBACK; gpt-5.6-sol+medium недоступен, используется gpt-5.6-terra+medium`.
После этого координатор использует `gpt-5.6-terra` с `medium`. Если свежих
сведений нет или нет обеих пар, основной Codex запускается с исходными
аргументами без принудительной модели и уровня рассуждений, а диагностика
показывает `DEGRADED`. `codex-native` остаётся явным диагностическим обходом.
Явно заданные `--model` и `model_reasoning_effort` сохраняются для основного
диалога; модели дочерних узлов всё равно выбирает контроллер. `codex help`,
`codex update` и другие служебные либо явно нативные формы выполняются как
обычный Codex без подготовки контроллера. Текст после `--` не является
служебной подкомандой: `codex -- help` и `codex -- update` остаются
пользовательскими запросами.

Для Ultra есть два способа. Первый запускает новый нативный процесс до
разрешателя и контроллера и передаёт исходные аргументы настоящему Codex:

```bash
codex -c 'model_reasoning_effort="ultra"'
```

Второй способ — сначала нативный обход:

```bash
codex-native
```

Затем в этой новой нативной сессии выбрать нужный уровень через `/model`.
Уже работающую управляемую сессию нельзя превратить в нативную через `/model`:
нужно выйти и запустить её заново. `pro` — план подписки, а не уровень
рассуждений. Ultra не участвует в дочерней маршрутизации: в дочерних
...
новый умный ход. Если аренду, контроллер или обработчики доказать не удалось,
старый умный маршрут не присоединяется, но пользовательский запрос продолжает
основной Codex без остановки `SessionStart` или `UserPromptSubmit`.
...
Тот же путь исправляет единственный допустимый вид постороннего изменения
активной активации: кэш Python `__pycache__` и файлы `*.pyc`, созданные для
существующих исходных файлов. Просмотр возвращает
`ACTIVATION_BYTECODE_REPAIR_REQUIRED` и точный перечень удаляемых файлов;
применение под установочной блокировкой повторно проверяет весь перечень,
удаляет только доказанный байткод и сверяет исходный отпечаток дерева. Любой
другой добавленный, изменённый либо переименованный объект закрывает
восстановление кодом `ACTIVATION_BYTECODE_REPAIR_UNSAFE`. Новые активации
публикуются с каталогами `0500`, обычными файлами `0400` и исполняемыми
файлами `0500`, а все поставляемые точки входа запрещают запись байткода.
...
|---|---|---|
| `CODEX_VERSION_INCOMPATIBLE` | Версия неканоническая, предварительная либо ниже `0.144.4` | Использовать канонический стабильный выпуск не ниже нижней границы; сам номер всё равно только допускает проверку возможностей |
...
| `SOURCE_LINEAGE_MISMATCH` | Устанавливаемые файлы изменены без обновления их линии | Не применять источник; увеличить поколение для намеренного нового выпуска и пересчитать отпечаток реализации |
| `SOURCE_DOWNGRADE_REJECTED` | Рабочая копия имеет поколение ниже уже установленного | Не повторять применение из этой копии; перейти на актуальную ветку |
| `SOURCE_LINEAGE_CONFLICT` | Один номер поколения связан с разными реализациями | Не выбирать победителя по времени или хешу; присвоить новой реализации следующее поколение |
| `PLUGIN_CACHE_MISSING`, `PLUGIN_CACHE_DRIFT` | Фактически загружаемый кеш отсутствует либо отличается от принятой активации | Не считать `doctor` успешным; повторить штатное применение из актуальной ветки и затем `--smoke --json` |
| `HOOK_CACHE_PROBE_FAILED`, `HOOK_CACHE_PROBE_INVALID`, `HOOK_CACHE_NOT_LOADED`, `HOOK_CACHE_DRIFT` | Codex не доказал загрузку хуков из проверенного кеша | Сохранить результат дымовой проверки и вывод `codex plugin list --json`; не править кеш вручную |
| `HOOK_CACHE_REPORTED_ERRORS` | `hooks/list` сообщил ошибки или предупреждения загрузки обработчиков | Не принимать дымовую проверку; изучить вывод `hooks/list`, исправить загрузку и повторить диагностику |
...
| `ACTIVATION_BYTECODE_REPAIR_REQUIRED` | Отпечаток активной активации отличается только из-за доказанного кэша Python | Выполнить установочные `--recover --preview --json`, затем совпавший `--recover --apply --json` |
| `ACTIVATION_BYTECODE_REPAIR_UNSAFE` | В активной активации найдено изменение, которое нельзя свести к кэшу Python | Ничего не удалять вручную; сохранить `--inspect --json` и проверить источник изменения |
...
- [живая проверка обновления и глобального запуска](../analysis/2026-08-04-codex-update-self-healing-validation.md);
- [историческая живая проверка маршрута субагента](../analysis/2026-07-20-adaptive-subagents-v2-validation.md);
