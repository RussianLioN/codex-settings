...
`codex`: псевдоним выставляет признак умного входа, `codex-highfd` поднимает предел
...
Статус на 14 августа 2026 года: документ описывает реализованного кандидата и
целевой пользовательский путь, но не доказывает включённую рабочую среду.
...
решения владельца команды с `--apply`, принятие хуков и согласование точки
входа ниже являются только будущими шагами изолированной приёмки и не должны
выполняться в текущем профиле.

## Быстрый старт

До закрытия `HOOK-INCIDENT-001` этот раздел читать как сценарий будущей
изолированной приёмки после снятия блокировки владельцем.
...
пользовательскими запросами. Если умный интерактивный контур не доказан,
основной Codex запускается автоматически с исходными аргументами, а
`codex-native` остаётся только диагностическим обходом. Уже открытый разговор нельзя задним
...
- [Готовность и автоматическое восстановление](../../docs/analysis/adaptive-subagents-v2-flow.md#готовность-и-автоматическое-восстановление)
  различает `READY`, начальный `UNAVAILABLE` и временный отказ после
  доказанного `SELECTED`.
...
Публичная диагностика возвращает `READY` только когда одновременно доказаны
выбранная пара координатора, успешная последняя проверка каталога и живой
командный сокет. `DEGRADED` означает, что основной Codex доступен, но полная
готовность умной части не доказана. Есть два разных случая:

- при начальном `UNAVAILABLE` доказанной пары ещё нет, поэтому корневой диалог
  запускается без принудительной модели и уровня рассуждений;
- после прежнего `SELECTED` последняя доказанная пара сохраняется и
  маршрутизация может продолжаться, но свежесть каталога временно не доказана.
...
В `extensions.diagnosis` читать `coordinatorReasonCode`,
`lastSuccessfulCheckAt` и `nextAttemptAt`. Это ограниченная проекция: она не
содержит пути, тексты исключений или содержимое каталога. После неудач один
фоновый цикл повторяет проверку через 5, 30, 120 и 300 секунд; после успеха —
через 300 секунд. `codex` и `codex resume --last` при `DEGRADED` продолжают
запуск основного Codex. Дымовая проверка полной умной готовности при этом не
выдаёт ложный успех.
...
новый умный ход. Ошибка аренды, обработчика или контроллера также не
останавливает пользовательский запрос: недоказанный старый маршрут не
присоединяется, а основной Codex продолжает ход без субагентов. Параметры
удалённого сервера, локального поставщика, произвольного профиля либо
конкурирующего механизма агентов не дают права на старый маршрут.
...
sequenceDiagram
    ...
    else Умный управляемый запуск недоступен
        W->>N: Основной Codex с исходными аргументами
        N-->>U: Обычный ход без умных значений по умолчанию
...
нет либо недоступны обе пары, основной Codex запускается без принудительной
модели и уровня рассуждений, а полная умная готовность получает состояние
`DEGRADED`. `codex-native` остаётся диагностическим обходом. Если пользователь явно
...
`SOURCE_UPDATE_CLEANUP_REQUIRED` запрещает недоказанную умную активацию, но
обычный запуск всё равно передаётся проверенному основному Codex. Такой случай
нельзя понижать до `RETRY_AFTER` и продолжать умную работу на снимке.
