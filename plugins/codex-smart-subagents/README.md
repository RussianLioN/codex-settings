...
Этот документ описывает предусмотренный пользовательский путь. Актуальный
отчёт живой проверки ещё не опубликован; зарезервированный путь —
...
Доказательство связано с одноразовым nonce конкретного запуска. При любом
расхождении запускается обычный ход: `UserPromptSubmit` не сохраняет
`TurnContext`, а `Stop` не требует недоступный инструмент. Эта ветвь считается
нормальным конечным выходом, а не ошибкой установки.
