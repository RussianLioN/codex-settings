...
Это исторический план проектирования и развёртывания. Актуальные операторские
действия, фактические ограничения локальной очереди, побочные действия
валидатора и безопасный откат описаны в
[руководстве по автономной работе](../guides/autonomous-workflow.md).

Части ниже сформулированы как целевое состояние. Реализована локальная очередь
SQLite, но не полный диспетчер с автоматическим возвратом аренды, повторными
попытками и надзирателем публикации. Внешние профили, роли и пользовательская
конфигурация не подтверждаются самим планом и проверяются заново перед
использованием.
