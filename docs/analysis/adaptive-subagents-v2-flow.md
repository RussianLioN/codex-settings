...
проверку. Актуальный отчёт такой проверки ещё не опубликован; его
...
```mermaid
flowchart TD
    A["Пользователь запускает codex-smart"] --> B{"Принятая активация и контроллер доказаны?"}
    B -->|нет, обычный путь доказан| C["Запустить обычный Codex с исходными параметрами"]
    B -->|нет, состояние неизвестно| Z["Остановить адаптивный запуск с кодом причины"]
    B -->|да| B1{"Версия Codex, bundled MCP и пользовательская политика доказаны?"}
    B1 -->|нет| C
    B1 -->|да| D["Запустить корневой диалог и bundled MCP с одноразовым nonce"]
    D --> D1["MCP отвечает точным tools/list и публикует живую аттестацию"]
    D1 --> E{"UserPromptSubmit повторно доказал политику, MCP и контроллер?"}
    E -->|нет| E0["Обычный ход без TurnContext и требования smart_plan"]
    E0 --> V
    E -->|да| E1["Сохранить ограниченный контекст хода"]
    E1 --> F["Корневой диалог вызывает smart_plan один раз"]
    ...
    U2 --> U3{"Stop повторно доказал политику, MCP и контроллер?"}
    U3 -->|нет| V
...
`codex-smart` задаёт модель корневого координатора только при запуске нового
диалога. В середине существующего диалога корневая модель не меняется.
Адаптивность достигается тем, что делегированный узел становится новым
процессом с собственной точной парой «модель — уровень рассуждения».

До первого хода оболочка проверяет три независимых основания адаптивного
режима. Первое — неизменяемый `.mcp.json` установленного расширения с точным
набором четырёх инструментов, обязательным запуском и режимом подтверждения
`approve`. Второе — неизменившаяся пользовательская политика Codex: она не
...
`UserPromptSubmit` и `Stop` заново проверяют эти основания и живой контроллер.
Если любое доказательство отсутствует, устарело или расходится, ход становится
обычным: контекст умного хода не создаётся, счётчик обязательного плана не
увеличивается, а завершение не блокируется. Поэтому сбой расширения не
превращается в цикл требований вызвать недоступный `smart_plan`. Только для
полностью доказанного хода `Stop` может выдать не более двух повторных
указаний, если корневой диалог ещё не создал план.
...
квитанции шлюз остаётся закрытым, но обычный Codex не блокируется. Обновление
